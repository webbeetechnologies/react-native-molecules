...
Use the `locale` prop to localize month/day labels in calendar UIs. The prop is typed as `DatePickerLocale` (`Intl.LocalesArgument`), so it accepts locale strings (for example, `"en"`) and locale arrays.
...
export default function Example() {
    ...
    return (
        <DatePicker.Provider value={date} onChange={setDate} locale="en">
            <DateField>
                <TextInput.Right>
                    <DatePicker.Trigger>
